...
<AccordionGroup>
  ...
  <Accordion title="Two Vetra Framework installs detected" icon="copy">
    You have an old `vetra-framework` beside a new one. Delete the old folder and restart.

    Do not try to fix this by overwriting. Garry's Mod merges every addon into one file
    system, so two copies are two copies of the same files, and one of them stands down.
    Which one stands down is not necessarily the one you would choose.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="An adapter says Unavailable" icon="plug-zap">
    Open the **Adapters** screen. Every unavailable adapter carries a sentence written by
    that adapter explaining why. The common ones:
  ...
  <Accordion title="My third-party adapter is not listed at all" icon="search-x">
    It did not register. Registration happens through one hook, once, after the map has
    loaded, and registration outside that window is refused.
